---
title: "Data Usage"
description: "Track and manage data consumption across your workspaces."
icon: "chart-column"
---

#### What data is sent to the OpenBug debugging service
...
The OpenBug debugging service does **not** directly read from your filesystem or log files; it only sees the data that the CLI sends in response to tool calls or as part of your chat.

#### Controlling data shared with the AI

You can control data sharing at multiple levels:

* **Per-service flags in `openbug.yaml`**:
...
The OpenBug dashboard ([`https://app.oncall.build`](https://app.oncall.build/)) shows:
...
Token usage is tied to the API key in `~/.openbug/config`, so using separate keys per environment or per team can make cost monitoring and attribution easier.
